...
public class StyleBuilderTest
{
	...
	public void ShouldBulidConditionalInlineStyles()
	{
		...
		string ClassToRender = new StyleBuilder( "background-color", "DodgerBlue" )
						...
						.Build();
		//assert
		_ = ClassToRender.Should().Be( "background-color:DodgerBlue;border-width:2px 20px 10px 4px;z-index:-1;padding:35px;" );
	}
	...
	public void ShouldAddExistingStyle()
	{
		...
		_ = StyleToRender.Should().Be( "background-color:DodgerBlue;;padding:35px;" );
		_ = StyleToRenderFromDefaultConstructor.Should().Be( "background-color:DodgerBlue;;padding:35px;;" );
	}
	...
	public void ShouldNotAddEmptyStyle()
	{
		...
		_ = StyleToRender.NullIfEmpty().Should().BeNull();
	}
	...
	public void ShouldAddNestedStyles()
	{
		...
		/// .foo { ;;;display:none;;;color:black;;; }
		/// Trimming is possible, but is it worth the operations for a non-issue?
		_ = StyleToRender.Should().Be( "background-color:DodgerBlue;padding:35px;z-index:-1;" );
	}
	...
	public void ShouldAddComplexStyles()
	{
		...
		/// .foo { ;;;display:none;;;color:black;;; }
		/// Trimming is possible, but is it worth the operations for a non-issue?
		_ = StyleToRender.Should().Be( "text-decoration:underline line-through;z-index:-1;" );
	}
	...
	{
		{
			...
			string StyleToRender = StyleBuilder.Empty()
							...
							.Build();
			//assert
			_ = StyleToRender.Should().Be( "background-color:black;" );
		}
	}
}